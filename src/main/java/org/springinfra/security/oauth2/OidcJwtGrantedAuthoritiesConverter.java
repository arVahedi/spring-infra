...
public class OidcJwtGrantedAuthoritiesConverter implements Converter<Jwt, Collection<GrantedAuthority>> {
    ...
    private Set<String> getAuthorities(Jwt jwt) {
        ...
        if (StringUtils.isNotBlank(this.authorityPostfix)) {
            authorities = authorities.stream().filter(item -> item.endsWith(this.authorityPostfix)).collect(Collectors.toSet());
        }
    ...
    private List<String> getResourceRoles(Jwt jwt) {
        ...
        if (resourceAccess != null
                && (clientResource = (Map<String, Object>) resourceAccess.get(this.clientId)) != null
